...
public class LeilaoDaoTests {
	...
	@Test
	public void deveRetornarLeiloesNaoEncerradosComNoMinimoTresLancesDados() {
		// criando as valor filtro
		Double inicio = 250.0;
		Double fim = 967.0;
		...
		// criando os leiloes, cada um com uma data
		Leilao leilao1 = new LeilaoBuilder().comDono(mauricio).comNome("XBox").comValor(250.0).constroi();

		leilao1.adicionaLance(new Lance(Calendar.getInstance(), mauricio, 150.0, leilao1));
		leilao1.adicionaLance(new Lance(Calendar.getInstance(), mauricio, 965.0, leilao1));
		leilao1.adicionaLance(new Lance(Calendar.getInstance(), mauricio, 251.0, leilao1));
		leilao1.adicionaLance(new Lance(Calendar.getInstance(), mauricio, 500.0, leilao1));

		// criando os leiloes, cada um com uma data
		Leilao leilao2 = new LeilaoBuilder().comDono(mauricio).comNome("Geladeira").comValor(300.0).constroi();

		leilao1.adicionaLance(new Lance(Calendar.getInstance(), mauricio, 300.0, leilao2));
		...
		// invocando o metodo para testar
		List<Leilao> leiloes = leilaoDao.disputadosEntre(inicio, fim);

		// garantindo que a query funcionou
		assertEquals(1, leiloes.size());
		assertEquals(250.0, leiloes.get(0).getValorInicial(), 0.0001);
		assertEquals("XBox", leiloes.get(0).getNome());
	}
	...
	@Test
	public void deveTrazerLeiloesDoUsuarioSemRepertir() {

		Usuario mauricio = new Usuario("Mauricio Aniche", "mauricio@aniche.com.br");

		// criando os leiloes
		Leilao leilao1 = new LeilaoBuilder().comDono(mauricio).comValor(3000.0)
				.comLance(Calendar.getInstance(), mauricio, 3000.0).comLance(Calendar.getInstance(), mauricio, 3100.0)
				.constroi();

		Leilao leilao2 = new LeilaoBuilder().comDono(mauricio).comValor(3200.0)
				.comLance(Calendar.getInstance(), mauricio, 3000.0).comLance(Calendar.getInstance(), mauricio, 3100.0)
				.comLance(Calendar.getInstance(), mauricio, 3200.0).comLance(Calendar.getInstance(), mauricio, 3300.0)
				.comLance(Calendar.getInstance(), mauricio, 3400.0).comLance(Calendar.getInstance(), mauricio, 3500.0)
				.constroi();
	...
	public void deveTrazerValorMedioDosLancesIniciaisDoUsuario() {
		...
		Leilao leilao2 = new LeilaoBuilder().comDono(mauricio).comValor(4000.0)
				.comLance(Calendar.getInstance(), mauricio, 3000.0).comLance(Calendar.getInstance(), mauricio, 3100.0)
				.comLance(Calendar.getInstance(), mauricio, 3200.0).comLance(Calendar.getInstance(), mauricio, 3300.0)
				.comLance(Calendar.getInstance(), mauricio, 4000.0).comLance(Calendar.getInstance(), mauricio, 3500.0)
				.constroi();
