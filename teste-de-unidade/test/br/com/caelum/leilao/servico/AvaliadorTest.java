...
public class AvaliadorTest {
	...
	public void naoDeveAvalisarSemNenhumLanceDado() {
	...
	public void calcularValorMediaDosLances() {
		// parte 1: cenario
		Leilao leilao = new CriadorDeLeilao().para("Pintura Leonardo da Vinci").lance(cesar, 300.0).lance(aline, 800.0)
				.lance(pedro, 699.0).constroi();
		// parte 2: acao
		double valorMedioCalculado = leiloeiro.getValorMedioLance(leilao);

		// parte 3: validacao
		double valorMedioEsperado = (leilao.getLances().get(0).getValor() + leilao.getLances().get(1).getValor()
				+ leilao.getLances().get(2).getValor()) / 3;
	...
	public void deveEncontrarOsTresMaioresLances() {
		Leilao leilao = new CriadorDeLeilao().para("Tablet").lance(joao, 100).lance(maria, 200).lance(joao, 300)
